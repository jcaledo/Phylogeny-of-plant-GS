...
* An alignment in phylip format (MCMCtree_GS_tutorial/data/representatives.phy) of 20 GS plant proteins from six representative plant species.

* A calibrated tree of the 20 proteins (MCMCtree_GS_tutorial/data/representatives.tre)
...
Note that `usedata = 0`tells MCMCtree to carry out sampling from the prior. Now, in the terminal browse to `MCMCtree_GS_tutorial/wb` and type: `mcmctree ../prior/representatives_prior.ctl`. Once MCMCtree has finished, the output files `mcmc_prior.txt`, `out_prior.txt`, and `FigTree_prior.tre`, which in this case contain the prior sample and prior summaries. We have move these files to the `MCMCtree_GS_tutorial/output` directory and they will be used to plot the calibration densities and the marginal prior.

```{r}
# Install the mcmc3r package if necessary:
if (!any(rownames(installed.packages()) == "mcmc3r")) install.packages("v")
library(mcmc3r, quietly = TRUE, warn.conflicts = FALSE)

prior <- read.csv2("./output/mcmc_prior.txt", sep = "\t", header = TRUE)
prior <- as.data.frame(sapply(prior, as.numeric))

# Plot prior root age (GS2/GS1):
plot(density(prior[,root]), lty = 2, main = "Prior root age") 
curve(mcmc3r::dB(x, 4.16, 10.42, 0.025, 0.025), n = 5e2, add = TRUE)

# Plot prior GS1a divergency:
gs1a <- which(names(pos1) == "t_n34")
plot(density(prior[,gs1a]), lty = 2, main = "Prior GS1a-divergence node age")  
curve(mcmc3r::dB(x, 3.062, 3.668, 0.025, 0.025), n = 5e2, add = TRUE)
```

As it can be appreciated, the marginal prior age (dashed lines) of both nodes are similar to their corresponding calibration densities (solid lines).
